...
class BiliComment:
    @staticmethod
    async def get_random_comment(aid, credential, type_=comment.CommentResourceType.VIDEO, page_index=1, order=comment.OrderType.LIKE):
        """随机获取几条热评，直接生成评论prompt string"""
        if str(aid).startswith("av"):
            aid = aid[2:]
        _LOGGER.debug(f"正在获取视频{aid}的评论列表")
        comment_list = await comment.get_comments(oid=aid, credential=credential, type_=type_, page_index=page_index, order=order)
        _LOGGER.debug(f"获取视频{aid}的评论列表成功")
        if len(comment_list) == 0:
            _LOGGER.warning(f"视频{aid}没有评论")
            return None
        _LOGGER.debug(f"正在随机选择评论")
        ignore_name_list = ["哔哩哔哩", "AI", "课代表", "机器人", "小助手", "总结"] # TODO 从配置文件中读取（设置过滤表尽可能避免低质量评论）
        new_comment_list = []
