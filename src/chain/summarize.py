...
class SummarizeChain:
    ...
    async def _check_require(self, at_items: AtItems, _uuid: str) -> bool:
        ...
        if (
            ...
            self.task_status_recorder.update_record(_uuid, stage=TaskProcessStage.END,
                                                    end_reason=TaskProcessEndReason.ERROR, gmt_end=int(time.time()),
    ...
    async def _process_video_info(self, at_items: AtItems, _uuid: str) -> bool | tuple:
        ...
        if self.cache.get_cache(key=video_info["bvid"]):
            ...
            self.task_status_recorder.update_record(_uuid, stage=TaskProcessStage.END,
                                                    end_reason=TaskProcessEndReason.NORMAL, gmt_end=int(time.time()))
            return True
    ...
    async def _get_subtitle_from_bilibili(self, video: BiliVideo, _uuid: str, format_video_name: str) -> bool | str:
    ...
    async def _send_reply(self, at_items: AtItems, resp: dict, video_info: dict, _uuid: str):
        ...
        self.task_status_recorder.update_record(_uuid, stage=TaskProcessStage.WAITING_PUSH_TO_CACHE)
    ...
    async def _send_private(self, at_items: AtItems, resp: dict, video_info: dict, _uuid: str):
    ...
    async def _on_start(self):
        ...
        oncomplete_task = []
        oncomplete_task += self.task_status_recorder.get_record_by_stage(TaskProcessStage.PREPROCESS,
                                                                         TaskProcessEvent.SUMMARIZE)
        oncomplete_task += self.task_status_recorder.get_record_by_stage(TaskProcessStage.WAITING_LLM_RESPONSE,
                                                                         TaskProcessEvent.SUMMARIZE)
        oncomplete_task += self.task_status_recorder.get_record_by_stage(TaskProcessStage.WAITING_SEND,
                                                                         TaskProcessEvent.SUMMARIZE)
        oncomplete_task += self.task_status_recorder.get_record_by_stage(TaskProcessStage.WAITING_PUSH_TO_CACHE,
                                                                         TaskProcessEvent.SUMMARIZE)
        oncomplete_task += self.task_status_recorder.get_record_by_stage(TaskProcessStage.WAITING_RETRY,
                                                                         TaskProcessEvent.SUMMARIZE)
            ...
            while True:
                ...
                if at_items.get("item").get("uuid", None) is not None:
                    _item_uuid = at_items["item"][
                        "uuid"]  # TODO 有uuid就一定有视频记录，但如果是继续处理的话，gmt_create时间就会不准确，要不要在读取时再修改一次？
                else:
                    if isinstance(at_items.get("item").get("private_msg_event", {"content": None}).get("content", None),
                                  Video):
                        temp = at_items
                        temp["item"]["private_msg_event"]["content"] = temp["item"]["private_msg_event"][
                            "content"].get_bvid()
                    else:
                        temp = at_items
                    _item_uuid = self.task_status_recorder.create_record(temp, TaskProcessStage.PREPROCESS,
                                                                         TaskProcessEvent.SUMMARIZE, int(time.time()))
                ...
                video_info = await video.get_video_info()
                format_video_name = f"『{video_info['title']}』"
                if data["stage"] == TaskProcessStage.PREPROCESS.value or data[
                    "stage"] == TaskProcessStage.WAITING_LLM_RESPONSE.value:
                    begin_time = time.perf_counter()
                    resp = await self._process_video_info(at_items, _item_uuid)
                    if type(resp) is bool:
                        continue
                    video, video_info, format_video_name, video_tags_string, video_comments = resp
                    # 处理视频音频流和字幕
                    _LOGGER.debug(f"视频信息获取成功，正在获取视频音频流和字幕")
                    if at_items["item"].get("whisper_subtitle", None) is not None:
                        ...
                        if len(video_info["subtitle"]["list"]) == 0:
                            resp = await self._get_subtitle_by_whisper(video_info, video, _item_uuid, format_video_name,
                                                                       at_items)
                            if type(resp) is bool:
                                continue
                            text = resp
                            ...
                            text = await self._get_subtitle_from_bilibili(video, _item_uuid, format_video_name)
                    ...
                    self.task_status_recorder.update_record(_item_uuid, stage=TaskProcessStage.WAITING_LLM_RESPONSE)
                    ...
                    self.task_status_recorder.update_record(_item_uuid, stage=TaskProcessStage.WAITING_SEND,
                                                            data=at_items)

                data = self.task_status_recorder.get_data_by_uuid(_item_uuid)
                if data["stage"] == TaskProcessStage.WAITING_SEND.value or data[
                    "stage"] == TaskProcessStage.WAITING_RETRY.value:
                    begin_time = time.perf_counter()
                        ...
                        try:
                            ...
                            if resp["noneed"] is True:
                                ...
                                if (
                                    ...
                                    _LOGGER.debug(f"该消息是私信消息，将结果放入私信处理队列")
                                    await self._send_private(at_items, resp, video_info, _item_uuid)
                                else:
                                    _LOGGER.debug(f"正在将结果加入发送队列，等待回复")
                                    await self._send_reply(at_items, resp, video_info, _item_uuid)


                        except Exception as e:
                            _LOGGER.error(f"处理结果失败：{e}，大概是ai返回的格式不对，尝试修复")
                            traceback.print_tb(e.__traceback__)
                            self.task_status_recorder.update_record(_item_uuid, stage=TaskProcessStage.WAITING_RETRY)
                            ...
                            if res is True:
                                self.task_status_recorder.update_record(_item_uuid, stage=TaskProcessStage.END,
                                                                        ...
                                                                        gmt_end=int(time.time()), if_retry=True)
                            elif res is False:
                                self.task_status_recorder.update_record(_item_uuid, stage=TaskProcessStage.END,
                                                                        end_reason=TaskProcessEndReason.ERROR,
                                                                        gmt_end=int(time.time()),
