...
def run(output_dir, data):
    ...
    plt.pie(list(end_reason_counts.values()), labels=list(end_reason_counts.keys()), autopct='%1.0f%%')
    plt.title('任务结束原因')
    ...
    plt.xticks(range(0, max_value + 1))
    for bar in bars:
        plt.text(bar.get_width() - 0.2, bar.get_y() + bar.get_height() / 2, str(int(bar.get_width())))
    plt.savefig(f"{output_folder}/用户发起任务次数排名竖状图.png")

    # Pie Chart for Request Types
    plt.figure(figsize=(4, 4))
    plt.pie(list(request_type_counts.values()), labels=list(request_type_counts.keys()), autopct='%1.0f%%')
    plt.title('请求类型占比')
...
if __name__ == '__main__':
    with open(r"填入你records.json位置", 'r', encoding='utf-8') as f:
        data = json.load(f)

    run(r"填入你希望输出的目录", data)
